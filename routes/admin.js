...
router.post('/admin_login',
  ...
  router.get("/adminDashboard",isLoggedIn,async(req,res)=>{
   ...
   try{
    ...
    res.render("adminDashboard",{foundNgo,user});
   }
   catch(error){
    console.log(error)
   }
  ...
  router.get("/adminHome",isLoggedIn,async(req,res)=>{
   try{
    let user=req.user.username;
    let foundNgo=await NGO.find({}).sort({_id:-1})
...
router.get("/addNgo",isLoggedIn,(req,res)=>{
res.render("add")
})
...
router.get("/adminview/:id",isLoggedIn,async(req,res)=>{
try{
    let{id}= req.params;
...
router.get("/adminedit/:id",isLoggedIn,async(req,res)=>{
try{
    let{id}= req.params;
    const ngoParticular=await NGO.findById(id);
